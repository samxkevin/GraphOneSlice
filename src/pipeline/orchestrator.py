...
async def run_repo_resolution(
    ...
    semaphore = asyncio.Semaphore(settings.arxiv_abs_max_concurrency)

    while True:
        papers = await storage.get_papers_by_status(
            "PARSED", settings.pipeline_batch_size
        )

        if not papers:
            break

        async def resolve_one(paper):
            paper_id = paper["id"]

            claimed = await storage.claim_paper(
                paper_id, "PARSED", "RESOLVING_REPO"
            )
            if not claimed:
                return

            # Only the external HTTP request is concurrency-limited.
            # Database writes and deterministic processing happen outside
            # the semaphore so the concurrency setting controls network load.
            async with semaphore:
                fetch_result = await evidence_adapter.fetch_abs_page(
                    paper["canonical_url"]
                )

            # Persist the RAW evidence itself, not just the HTTP status.
            # This is the exact HTML the extractor below reads from.
            # The abstract-page fetch is 1:1 with this paper, so the scalar
            # paper_id column is correct here.
            ...
            await storage.log_event(
                stage="repo_evidence_fetch",
                status=fetch_result.fetch_status,
                source="arxiv",
                record_id=paper_id,
            ...
            if selected is not None:
                match = next(
                    ...
                    if c["repo_url"] == selected.repo_url
                )
                await storage.select_repo_link(match["id"])

            await storage.claim_paper(
                paper_id, "RESOLVING_REPO", "RESOLVED"
            )

            await storage.log_event(
                stage="repo_resolution",
                status="OK",
                source="arxiv",
                record_id=paper_id,
                detail={
                    "candidates": len(candidates),
                    "selected": selected.repo_url if selected else None,
                },
            )

        await asyncio.gather(*(resolve_one(paper) for paper in papers))
...
async def run_github_verification(settings: Settings, storage: Storage) -> None:
    ...
    while True:
        papers = await storage.get_resolved_papers_without_github_snapshot(
        ...
        for paper in papers:
            ...
            if selected is not None:
                snapshot = await client.verify_and_get_stars(
                    selected["repo_url"]
                )
                await storage.record_github_snapshot(snapshot)
                await storage.log_event(
                    stage="github_verify",
                    status=snapshot.api_status.value,
                    source="github",
                    record_id=paper_id,
                    detail={
...
async def run_validation(settings: Settings, storage: Storage) -> None:
    ...
    while True:
        papers = await storage.get_papers_by_status(
            "RESOLVED", settings.pipeline_batch_size
        )
        ...
        for paper in papers:
            ...
            try:
                ...
                snapshot = None
                if selected is not None:
                    snapshot = await storage.get_latest_github_snapshot(
                        selected["repo_url"]
                    )

                payload = validate_and_build_export(
                    parsed, selected, snapshot
                )
                ...
                await storage.log_event(
                    ...
                    source="arxiv",
                    record_id=paper_id,
                    error_type="PaperValidationError",
                ...
                await storage.log_event(
                    ...
                    source="arxiv",
                    record_id=paper_id,
                    error_type="ParsedPaperValidationError",
